...
const addProduct = {
    AddCartProductS: (data) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            const checkQuery = `SELECT * FROM CartItems WHERE userId = ? AND productId = ?`;
            db.query(checkQuery, [userId, productId], (checkError, checkResults) => {
                ...
                if (checkResults.length > 0) {
                    ...
                    const insertQuery = `INSERT INTO CartItems (userId, productId, price, size, color, productname, image, dummyprice)
                                         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    findByAddCartId: async (userId) => {
        return new Promise((resolve, reject) => {
            const query = 'SELECT * FROM CartItems WHERE userId = ?';
